...
# 使用服务器保存的陈旧模型更新，用乘10的n次方量化，并且延迟客户端的选择上，不是每轮都有四个八个客户端延迟而改为
# 最多有四个或八个等等个延迟，不聚合延迟轮次大于threshold的延迟模型，使用she加密
import os
import argparse
import random
import time
...
if __name__ == "__main__":
    ...
    for i in range(args['num_comm']):
        ...
        for m in range(len(iiid)):
            ...
            for key, var in local_parameters.items():
                keys.append(key)
                local_gradients.append(round(local_parameters[key].numpy()*10000000))
                # local_gradients.append(local_parameters[key].numpy() - global_parameters[key].numpy())
            # k = keys
            print(local_gradients)
            #SHE加密
