# 这个是最基础的异步方案，只进行了加权更新，没有添加mu项，没有用服务器保存的陈旧模型参与聚合
import os
import argparse
...
from Models import Mnist_2NN, Mnist_CNN, Cifar_CNN
...
if __name__ == "__main__":
    ...
    percentageOfStale = 0.4  # 表示延迟客户端所占总客户端数目的百分比
    ...
    for i in range(args['num_comm']):
        ...
        iiid = [key for key, value in enumerate(flag) if value == 1]
        print("参与此轮训练的所有客户端：{}".format(iiid))
        time = []
        for b in range(20):
        ...
        for m in range(len(iiid)):
            ...
            if i - time_stamp[iiid[m]] == 1 or i == 0:
                local_parameters = myClients.clients_set[clients_in_comm[iiid[m]]].localUpdate(args['epoch'], args['batchsize'], net,
            ...
            # 对于上一轮没参加训练这一轮参加训练的客户端，说明该客户端上传的模型是延迟的模型
                ...
                stale_global_parameters = torch.load(os.path.join(args['save_path'],
                                         '{}_num_comm{}'.format(args['model_name'], time_stamp[iiid[m]]-1)))
                local_parameters = myClients.clients_set[clients_in_comm[iiid[m]]].localUpdate(args['epoch'], args['batchsize'], net,
                                                                             loss_func, opti, stale_global_parameters)
            # 加权聚合
            ...
            time_stamp[iiid[m]] = i + 1  # 设置客户端的时间戳为当前训练轮次
        ...
        if (i + 1) % args['save_freq'] == 0:
            ...
            #                                                                                     args['num_of_clients'],
            #                                                                                     args['cfraction'])))
        np.savetxt('AsyAccuracy(staleThreshold={},percent={},FedAvg&straggler&fmnist).txt'.format(stale_threshold, percentageOfStale), AsyAccuracy)