# 这个是改进版的AsyFL方案，没有使用加权更新，增加了落后客户端的本地迭代轮次，
# 也添加了mu项，使用服务器保存的陈旧模型更新，但没有进行量化和添加掩码
import os
import argparse
...
if __name__ == "__main__":
    ...
    threshold = 2  # 使用未参与本轮训练的客户端的陈旧模型参与聚合，这是陈旧阈值。
    ...
    for i in range(args['num_comm']):
            ...
            # 对于上一轮没参加训练这一轮参加训练的客户端，说明该客户端上传的模型是延迟的模型
                ...
                stale_global_parameters = torch.load(os.path.join(args['save_path'],
                                                                  '{}_num_comm{}'.format(args['model_name'],
                                                                                         time_stamp[iiid[m]] - 1)))
                local_parameters = myClients.clients_set[clients_in_comm[iiid[m]]].localUpdate(args['epoch']+2,
                                                                                               args['batchsize'], net,
                                                                                               loss_func, opti,
        ...
        if (i + 1) % args['save_freq'] == 0:
            ...
            #                                                                                     args['num_of_clients'],
            #                                                                                     args['cfraction'])))
        r = os.getcwd() + "\\nonIID\FedProx_AsyAccuracy_PPFL"
        np.savetxt(r + '\FedProx_AsyAccuracy_Pro(staleThreshold={},percent={},mu={},threshold={},20stale,7local,newnewWeight).txt'.format(
