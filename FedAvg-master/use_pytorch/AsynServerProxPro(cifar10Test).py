# 这个是AsyFL方案，加权更新，使用服务器保存的陈旧模型参与聚合，添加了mu项，
# 还没有计算欧几里得距离,换模型与数据集进行测试，使用cifar10数据集与
import os
import argparse
...
if __name__ == "__main__":
    ...
    for i in range(args['num_comm']):
        ...
        if (i + 1) % args['save_freq'] == 0:
            ...
            #                                                                                     args['num_of_clients'],
            #                                                                                     args['cfraction'])))
        r = os.getcwd() + "\\nonIID\FedProx_AsyAccuracy_Pro\cifar10-cnn"
        np.savetxt(r + '\cnn2_FedProx_AsyAccuracy_Pro(staleThreshold={},percent={},mu={},threshold={},20stale,newnewWeight).txt'.format(
