    ...
    for i in range(args['num_comm']):
        ...
        for m in range(len(iiid)):
            ...
            for key, var in local_parameters.items():
                ...
                # print(shape)
                local_parameters[key] = np.array(local_parameters[key]).flatten() .tolist()  # 将当前层展平以便逐个加密
                #  开始加密
                for j in range(len(local_parameters[key])):
                    local_parameters[key][j] = she_enc(p, L, local_parameters[key][j])  # 逐个加密
                # local_parameters[key] = np.reshape(local_parameters[key], np.array(a))  # 恢复初始维度
        ...
        for key, var in sum_parameters.items():
            ...
            sum_parameters[key] = np.reshape(sum_parameters[key], shape[idex])  # 恢复初始维度
            idex += 1
