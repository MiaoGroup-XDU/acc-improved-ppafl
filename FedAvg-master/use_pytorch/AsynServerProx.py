# 这个进行了加权更新并且添加了mu项，但没有用服务器保存的陈旧模型参与聚合
import os
import argparse
...
if __name__ == "__main__":
    ...
    for i in range(args['num_comm']):
        ...
        for m in range(len(iiid)):
            ...
            if i - time_stamp[iiid[m]] == 1 or i == 0:
                local_parameters = myClients.clients_set[clients_in_comm[iiid[m]]].localUpdate(args['epoch'], args['batchsize'], net,
                                                                         loss_func, opti, global_parameters, args["mu"])
            # 对于上一轮没参加训练这一轮参加训练的客户端，说明该客户端上传的模型是延迟的模型
                ...
                local_parameters = myClients.clients_set[clients_in_comm[iiid[m]]].localUpdate(args['epoch'], args['batchsize'], net,
                                                                             loss_func, opti, stale_global_parameters, args["mu"])
            # 加权聚合
        ...
        np.savetxt(r+'\FedProx_AsyAccuracy(staleThreshold={},percent={},mu={},newWeight).txt'.format(stale_threshold, percentageOfStale, args["mu"]), AsyAccuracy)